...
class Zp365GgSpider(CrawlSpider):

    name = 'zp365_gg'
    allowed_domains = ['gg.zp365.com',]
    ...
    def __init__(self):
        ...
        # http://gg.zp365.com/News/News483_423280.shtml
        self.rules = (
            Rule(LinkExtractor(allow=('/News/News\d{3}_\d{6}\.shtml',)), callback='parse_page'),
        )

        super(Zp365GgSpider, self).__init__()

    def parse_page(self, response):

        url = response.url
        if url.startswith('http://gg.zp365.com/News/'):
